...
def login(user:UserBody):
    ...
    print(user)
    user_id,username= login_user(db_uri=MONGO_URI,db_name="diseaseDetector",collection_name="users",document=user)
...
def signUp(user:UserBody):
    ...
    user_id= create_user(db_uri=MONGO_URI,db_name="diseaseDetector",collection_name="users",document=user)
    if user_id != False:
